...
const BRAND = ["brand", "brand-hover", "brand-active", "brand-soft", "brand-ring"];
...
const TYPE_SCALE: Array<[string, string, number, number, number, string]> = [
  ...
  ["text-lg",   "text-[17px]", 17, 500, 1.40, "-0.005em"],
  ["text-xl",   "text-xl",   20, 600, 1.35, "-0.01em"],
  ["text-2xl",  "text-2xl",  24, 600, 1.25, "-0.015em"],
  ["text-3xl",  "text-3xl",  30, 600, 1.20, "-0.02em"],
  ["text-4xl",  "text-4xl",  40, 600, 1.10, "-0.025em"],
];
    ...
    <div className="h-full w-full overflow-y-auto bg-[var(--surface-1)] text-[var(--text)]">
      ...
      <header className="sticky top-0 z-30 border-b border-[var(--border)] bg-[var(--surface-2)]/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1200px] flex-wrap items-center gap-x-6 gap-y-2 px-6 py-2">
          <div className="text-xs text-[var(--text-muted)]">DESIGN.md showcase — tune the knobs live</div>
          <div className="ml-auto flex flex-wrap items-center gap-x-4 gap-y-2">
            <KnobSlider label="hue" value={hue} min={0} max={360} step={1} onChange={setHue} display={`${hue}°`} onReset={() => setHue(KNOB_DEFAULTS.hue)} isDefault={hue === KNOB_DEFAULTS.hue} />
            <KnobSlider label="chroma" value={chroma} min={0} max={0.37} step={0.005} onChange={setChroma} display={chroma.toFixed(3)} onReset={() => setChroma(KNOB_DEFAULTS.chroma)} isDefault={chroma === KNOB_DEFAULTS.chroma} />
            <KnobSlider label="contrast" value={contrast} min={0} max={1} step={0.05} onChange={setContrast} display={contrast.toFixed(2)} onReset={() => setContrast(KNOB_DEFAULTS.contrast)} isDefault={contrast === KNOB_DEFAULTS.contrast} />
            {mounted && (
              <Button
                size="sm"
                variant="outline"
      ...
      <main className="mx-auto max-w-[1200px] space-y-16 px-6 py-10">
            ...
            <div className="flex flex-col gap-2">
              {TEXTS.map((t) => (
                <div key={t} className="flex items-baseline gap-4">
                  <span className="w-32 font-mono text-xs text-[var(--text-muted)]">--{t}</span>
              ...
              <div className="flex items-baseline gap-4">
                <span className="w-32 font-mono text-xs text-[var(--text-muted)]">--text-on-accent</span>
                <span
              ...
              <div className="flex items-baseline gap-4">
                <span className="w-32 font-mono text-xs text-[var(--text-muted)]">--text-on-danger</span>
                <span
                  className="rounded-md px-2 py-1 text-base"
                  style={{ background: "var(--danger)", color: "var(--text-on-danger)" }}
                >
                  on danger fill
                </span>
              </div>
        ...
        <Section id="typography" title="Typography" specRef="DESIGN.md §3">
          <div className="flex flex-col gap-3">
            {TYPE_SCALE.map(([token, , px, weight, lh, tracking]) => (
              <div key={token} className="grid grid-cols-[10rem_1fr] items-baseline gap-4 border-b border-[var(--border)] pb-3 last:border-0">
            ...
            <div className="grid grid-cols-[10rem_1fr] items-baseline gap-4 pt-2">
              ...
              <div className="font-mono text-sm">128.00 BPM · A♭ minor · 06:42 · /Users/dj/library/track.aiff</div>
            </div>
          </div>
        </Section>
          ...
          <div className="flex flex-wrap items-end gap-6">
            {RADIUS_STEPS.map((r) => (
              <div key={r} className="flex flex-col items-center gap-2">
                ...
                />
                <div className="font-mono text-xs text-[var(--text-muted)]">--radius-{r}</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section id="borders" title="Borders" specRef="DESIGN.md §6">
          <div className="flex gap-4">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="font-mono text-xs text-[var(--text-muted)]">--border</div>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="size-24 rounded-md border border-[var(--border-strong)] bg-[var(--surface-2)]" />
              <div className="font-mono text-xs text-[var(--text-muted)]">--border-strong</div>
            </div>
          </div>
        </Section>
          ...
          <div className="flex flex-wrap gap-8 py-6">
            {SHADOW_STEPS.map((n) => (
              <div key={n} className="flex flex-col items-center gap-3">
                ...
                />
                <div className="font-mono text-xs text-[var(--text-muted)]">--shadow-{n}</div>
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section id="motion" title="Motion" specRef="DESIGN.md §8">
          <p className="mb-4 text-sm text-[var(--text-muted)]">Hover any tile.</p>
        ...
        <Section id="components" title="Components" specRef="DESIGN.md §9">
          <SubSection title="Buttons — in-app action hierarchy">
            <p className="mb-3 text-xs text-[var(--text-muted)]">
              In-app pages use <code className="font-mono">variant=&quot;ghost&quot;</code> for everything. Hierarchy is carried by text color and state, not by switching variants. Solid <code className="font-mono">default</code> is reserved for marketing surfaces and confirmation dialogs.
            </p>
            <div className="flex flex-wrap items-center gap-2 rounded-md border border-[var(--border)] bg-[var(--surface-2)] p-3">
              <Button variant="ghost" size="sm" className="h-7 text-xs gap-1.5 text-primary hover:bg-primary/10 hover:text-primary">
                Save all (3)
              </Button>
              <Button variant="ghost" size="sm" className="h-7 text-xs gap-1.5 text-emerald-600 hover:bg-emerald-600/10 hover:text-emerald-600">
                Apply rules
              </Button>
              <Button variant="ghost" size="sm" className="h-7 text-xs gap-1.5 text-muted-foreground hover:text-foreground">
                Auto-fill
              </Button>
              <Button variant="ghost" size="sm" className="h-7 text-xs gap-1.5 text-muted-foreground/40" disabled>
                Save all
              </Button>
            </div>
            <p className="mt-2 text-[10px] text-[var(--text-muted)]">
              Primary (brand) · Finalize (emerald) · Secondary (muted) · Disabled primary (kept in slot, muted)
            </p>
          </SubSection>
          <SubSection title="Buttons — variant gallery">
            <p className="mb-3 text-xs text-[var(--text-muted)]">
              All shadcn variants for reference. Inside the app, prefer <code className="font-mono">ghost</code> per the table above.
            </p>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>14 new tracks from artists you follow</CardDescription>
                </CardHeader>
                <CardContent className="text-sm text-[var(--text-muted)]">
                  Generated for week 16 · auto-synced from SoundCloud.
                </CardContent>
                <CardFooter>
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-primary hover:bg-primary/10 hover:text-primary">Open</Button>
                </CardFooter>
              </Card>
              <Card>
                <CardHeader>
                ...
                <CardContent className="text-sm">
                  ...
                  <div className="flex justify-between border-b border-[var(--border)] py-1.5">
                    <span className="text-[var(--text-muted)]">Missing metadata</span>
              ...
              <Card>
                ...
                <CardFooter className="gap-2">
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-primary hover:bg-primary/10 hover:text-primary">Publish</Button>
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-muted-foreground hover:text-foreground">Discard</Button>
                </CardFooter>
              </Card>
            </div>
          </SubSection>
          ...
          <SubSection title="Table (DESIGN.md §9.5)">
            <div className="overflow-hidden rounded-md border border-[var(--border)]">
              <Table>
                ...
                <TableBody>
                  {[
                    { sel: false, title: "Northern Drift",     artist: "Auval",        genre: "Melodic Techno",  bpm: "122.0", key: "F♯m",  len: "06:42" },
                    { sel: true,  title: "Lower Quay",         artist: "Mira Sand",    genre: "Deep House",      bpm: "118.5", key: "A♭m",  len: "07:18" },
                    { sel: false, title: "Outline of a Storm", artist: "Hiver",        genre: "Progressive",     bpm: "124.0", key: "C♯m",  len: "08:02" },
                    { sel: false, title: "Glassgrove",         artist: "Lurke",        genre: "Organic House",   bpm: "120.0", key: "Em",   len: "06:11" },
                    { sel: false, title: "Pale Engine",        artist: "Vector / Two", genre: "Techno",          bpm: "128.0", key: "Gm",   len: "07:44" },
                  ].map((r) => (
                    <TableRow key={r.title} data-state={r.sel ? "selected" : undefined}>
                      <TableCell className="font-medium">{r.title}</TableCell>
                      <TableCell className="text-[var(--text-muted)]">{r.artist}</TableCell>
                      <TableCell className="text-[var(--text-muted)]">{r.genre}</TableCell>
                      <TableCell className="text-right tabular-nums">{r.bpm}</TableCell>
                      <TableCell className="text-right tabular-nums">{r.key}</TableCell>
                      <TableCell className="text-right tabular-nums">{r.len}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
              ...
              {["Track A", "Track B (selected)", "Track C"].map((t, i) => (
                <div
                  ...
                  className={`flex items-center justify-between border-b border-[var(--border)] px-3 py-2 text-sm last:border-0 ${
                    i === 1 ? "bg-[var(--brand-soft)]" : "hover:bg-[var(--surface-3)]"
                  }`}
                >
                  <span>{t}</span>
                  <span className="font-mono text-xs text-[var(--text-muted)]">128.0 · A♭m</span>
                </div>
              ))}
            </div>
          </SubSection>
        ...
        <Section id="layout" title="Layout (inverted L)" specRef="DESIGN.md §10">
          <div className="flex h-64 overflow-hidden rounded-lg border border-[var(--border)]">
            ...
            <div className="flex flex-1 flex-col">
              ...
              <div className="flex-1 bg-[var(--surface-1)] p-4">
                <div className="text-sm text-[var(--text-muted)]">Main view (scrolls).</div>
              </div>
            </div>
          </div>
        </Section>
  ...
  return (
    <section id={id} className="space-y-6">
      <div className="flex items-baseline justify-between border-b border-[var(--border)] pb-2">
        ...
        <span className="font-mono text-xs text-[var(--text-muted)]">{specRef}</span>
      </div>
...
function SubSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      <h3 className="text-xs font-medium uppercase tracking-[0.04em] text-[var(--text-muted)]">{title}</h3>
      {children}
    </div>
  );
}

function SwatchRow({ names, bordered = false }: { names: string[]; bordered?: boolean }) {
  return (
    <div className="flex flex-wrap gap-3">
      {names.map((name) => (
        <div key={name} className="flex flex-col items-center gap-2">
          ...
          />
          <div className="font-mono text-xs text-[var(--text-muted)]">--{name}</div>
        </div>
      ))}
    </div>
  );
...
function KnobSlider({
  ...
  return (
    <div className="flex items-center gap-2">
      <label className="font-mono text-xs text-[var(--text-muted)]">{label}</label>
      <div className="w-32">
        <Slider
          value={[value]}
          min={min}
