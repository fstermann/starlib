...
import type { MusicSource, SourceTrack, SourceMetadata } from './types';
...
function extractMetadata(track: SourceTrack): SourceMetadata {
  ...
  if (raw.release_year && raw.release_year > 0) {
    const m = raw.release_month && raw.release_month > 0 ? raw.release_month : 1;
    ...
    const normalized = raw.created_at.replace(/\//g, '-').replace(' ', 'T').replace(' +0000', 'Z');
