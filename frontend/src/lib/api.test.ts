...
describe('fetchApi', () => {
  it('returns parsed JSON on success', async () => {
    ...
    const result = await fetchApi<{ id: string; name: string }>('/api/rulesets/active');
...
describe('api.getAppSettings', () => {
  it('calls GET /api/settings', async () => {
    const fetchSpy = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ preferred_output_format: 'aiff' }), { status: 200 })
    );
...
describe('api.updateAppSettings', () => {
  it('calls PUT /api/settings with body', async () => {
    const fetchSpy = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ preferred_output_format: 'mp3' }), { status: 200 })
    );
    vi.stubGlobal('fetch', fetchSpy);

    const result = await api.updateAppSettings({ preferred_output_format: 'mp3' });
    expect(result.preferred_output_format).toBe('mp3');

    const [, options] = fetchSpy.mock.calls[0];
    expect(options.method).toBe('PUT');
    expect(JSON.parse(options.body)).toEqual({ preferred_output_format: 'mp3' });
  });
});
...
describe('api.getRulesets', () => {
  it('returns rulesets response', async () => {
    const payload = {
      rulesets: [{ id: 'classic', name: 'Classic', is_builtin: true, rules: [] }],
...
describe('api.createRuleset', () => {
  it('sends POST with name and rules', async () => {
    const created = { id: 'new-id', name: 'My Workflow', is_builtin: false, rules: [] };
    const fetchSpy = vi.fn().mockResolvedValue(
