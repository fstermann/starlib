...
function makeSilentWav(): Buffer {
  ...
  buf.write("RIFF", off);
  off += 4;
  buf.writeUInt32LE(36 + numSamples, off);
  off += 4;
  buf.write("WAVE", off);
  off += 4;
  buf.write("fmt ", off);
  ...
  off += 2; // bits per sample
  buf.write("data", off);
  off += 4;
  buf.writeUInt32LE(numSamples, off);
  off += 4;
...
test.describe("Library waveform visibility", () => {
  ...
  test("waveform is hidden when switching collection modes", async ({
    page,
  }) => {
...
test.describe("Library track playback", () => {
  ...
  test("play button becomes enabled after selecting a track", async ({
    ...
    await expect(page.getByTestId("waveform-player")).toBeVisible();
    await expect(page.getByTestId("player-toggle")).toBeEnabled({
      timeout: 10_000,
    });
    await expect(page.getByTestId("player-toggle")).toHaveAttribute(
      "aria-label",
      "Play",
    );
  });
