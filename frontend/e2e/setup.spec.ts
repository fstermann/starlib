...
test.describe("Setup page", () => {
  test("shows setup form with required fields", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Welcome to SoundCloud Tools/i }),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: /Save and get started/i }),
    ).toBeVisible();
  });

  test("music folder has default value", async ({ page }) => {
    await page.goto("/setup");
    await expect(page.getByLabel(/Music folder/i)).toHaveValue(
      "~/Music/tracks",
    );
  });
