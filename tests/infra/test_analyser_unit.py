...
from backend.infra.analyser.cache import _atempo_chain
from backend.infra.analyser.pipeline import (
    _summarise_section_for_shazam,
    select_pitch_offsets,
)
from backend.infra.analyser.shazam import TokenBucket
...
class TestSelectPitchOffsets:
    ...
    def test_range_strategy_fans_out_across_band(self) -> None:
        ...
        assert sorted_offsets[0] == pytest.approx(12.0 * math.log2(120.0 / 128.0), abs=1e-6)
        assert sorted_offsets[2] == pytest.approx(12.0 * math.log2(130.0 / 128.0), abs=1e-6)
        # First-tried offset is the band midpoint shift.
        assert offsets[0] == pytest.approx(12.0 * math.log2(125.0 / 128.0), abs=1e-6)
    ...
    def test_range_strategy_dedupes_tight_band(self) -> None:
        ...
        assert len(offsets) == 1
        assert offsets[0] == pytest.approx(12.0 * math.log2(127.5 / 130.0), abs=1e-6)
