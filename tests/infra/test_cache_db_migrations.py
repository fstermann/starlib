...
def test_adopts_unstamped_analyser_schema_without_losing_jobs(tmp_path: Path) -> None:
    """A PR-era analyser DB stamped at 0004 upgrades without recreating tables."""
    ...
    conn.execute(
        ...
        VALUES ('preserved-job', 'complete', '{}', 1, 1)"""
    )
    conn.commit()
    conn.close()
