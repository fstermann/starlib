...
from backend.infra.analyser.shazam import ShazamMatch, _parse_shazamio_response
from backend.infra.db import engine as db_engine
from backend.infra.db.migrations import run_migrations
from backend.schemas.analyser import (
    JobCompleteEvent,
    WindowBpmEvent,
)
...
def _temp_db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[Path]:
    ...
    monkeypatch.setattr(
        "backend.services.analyser.controller.binary_locator.find_analyser_binary",
        lambda: "/dev/null",
    )
    yield db_path
    engine.dispose()
...
def _stub_subprocess(monkeypatch: pytest.MonkeyPatch, *, scripts: list[list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    monkeypatch.setattr(
        "backend.services.analyser.controller.run_analyser_subprocess",
        fake_run,
    )
    monkeypatch.setattr(
        "backend.services.analyser.controller.cache.make_shazam_slice",
        _fake_make_slice,
    )
    return captured
...
async def test_null_title_cached_row_does_not_block_retry(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from backend.services.analyser import start_shazam_scan
...
async def test_concurrent_fetch_set_audio_runs_ffmpeg_once(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from backend.infra.analyser import cache
    ...
    monkeypatch.setattr("backend.infra.analyser.cache.asyncio.create_subprocess_exec", fake_exec)
    monkeypatch.setattr("backend.infra.analyser.cache._find_ffmpeg", lambda: "/bin/true")
...
async def test_fetch_set_audio_hides_partial_file_until_complete(
    ...
    assert seen_during_download == [None], "cached_set_path leaked the partial file during download"
