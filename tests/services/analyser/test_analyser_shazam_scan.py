...
from backend.infra.analyser.shazam import ShazamMatch
...
from backend.services.analyser.controller import (
    _aggregate_timeline,
    _build_scan_grid,
)
...
async def _fake_make_slice(**kwargs: Any) -> Path:
...
class TestBuildSectionGrid:
    ...
    def test_samples_interior_by_spacing_avoiding_boundaries(self) -> None:
        from backend.services.analyser.controller import _build_section_grid
...
async def test_cancel_shazam_scan_stops_at_next_point(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from backend.services.analyser import cancel_shazam_scan, start_shazam_scan
...
class TestGridMinusRanges:
    def test_no_ranges_returns_grid_unchanged(self) -> None:
        from backend.services.analyser.controller import _grid_minus_ranges
