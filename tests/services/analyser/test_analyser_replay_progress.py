...
def test_scan_started_replays_after_cached_scans() -> None:
    ...
    state.active_shazam_scan = ShazamScanStartedEvent(job_id="job-1", tier="sweep", total_points=10, completed_points=3)
    ...
    assert started_idx > last_scan, (
        "scan_started must replay after cached scan rows so the frontend doesn't count history into the active run"
    )
...
def test_replayed_scans_carry_preview_and_artwork() -> None:
    _seed_job()
    _seed_scan("job-1", 0.0, preview_url="https://cdn/p.m4a", artwork_url="https://cdn/a.jpg")
    state = _JobState(
        job_id="job-1",
        options=analyser_controller.AnalyserJobOptions(),
        duration_s=600.0,
...
def test_reanalyse_marker_replays_before_windows() -> None:
    _seed_job()
    analyser_db.upsert_window_bpm(job_id="job-1", start_s=0.0, end_s=30.0, bpm=128.0, confidence="high")
    state = _JobState(
        job_id="job-1",
        options=analyser_controller.AnalyserJobOptions(),
        duration_s=600.0,
    )
    state.active_reanalyse = ReanalyseStartedEvent(job_id="job-1", ranges=[{"start_s": 0.0, "end_s": 100.0}])
    events = _replay_in_progress_state(state)
    types = [e.type for e in events]
